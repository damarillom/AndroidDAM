...
public class Parser extends AppCompatActivity {
    ...
    public List<Temp> parsejaJSon(String json) throws JSONException {
        ...
        try {
            ...
            JSONArray resultatJSON=respuestaJSON.getJSONArray("list");
            if (resultatJSON.length()>0) {
                Integer i=0;
                while (i<resultatJSON.length()) {
                    temperature=resultatJSON.getJSONObject(i).getString("main");
                    Log.d("test", temperature);
                    //accedir al nivel següent anterior...




                    time=resultatJSON.getJSONObject(i).getString("dt_txt");
